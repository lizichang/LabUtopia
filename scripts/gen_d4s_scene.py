# -*- coding: utf-8 -*-
"""生成 d4s_alkali_reagent.usd —— D4-S 固体样品 + 碱性试剂滴加反应场景（烘平自包含）。

D4-S = D2-S 把「洗瓶蒸馏水」换成「胶头滴管滴加碱性试剂」：
  - 挖粉动作完全不变：药匙/表面皿/粉末/试管/试管架 相对位置与 d2s **逐字一致**（用户
    要求，机械臂与粉末/试管相对位置都不变，挖粉轨迹才不会跑偏）
  - 洗瓶 (0.370,0.525) 位置改成 碱滴管（进主试管架空孔，与药匙同架）+ 碱瓶（原洗瓶附近）
  - 现象：白粉在管底 → 碱逐滴滴入 → 气泡/沉淀/液体变色（同 d3l 碱现象，几何实现）

基于 lab_001.usd 副本（d2s 同款：白名单删器材 + 抬台面 0.80 + 烘平自包含），叠加 d3l 的
滴管/试管玻璃透明化 + 气泡/变色液柱预烘焙。**碱瓶（alkaline_bottle.usd）是塑料瓶不玻璃化**
（diffuse op1.0 不透明，正常试剂瓶，液面被瓶身挡住本就看不见）；其自带 rubber_stopper
橡胶塞**翻放到桌面倒放**（同 d4l 模式，gen 静态摆，无拔塞动作）。

布局（d2s 实测坐标 2026-08-14 起，2026-08-25 碱瓶/滴管新增，2026-08-26 药匙/滴管同移第一列；
台面顶 z=0.80）：
  ...
  DropperAlkali   (0.659, 0.4002, 0.806)  碱滴管立插主试管架第一列第5排（2026-08-26 三次改位：①右后远端
                                         0.6993,0.4804 实测 IK 够不着→②第二列第3排与药匙同列相邻穿模→
                                         ③"药匙第一列第三排、滴管第一列第5排、第二列不放物品"=本布局）
  AlkaliBottle     (0.370, 0.30)  碱性试剂瓶（塑料瓶+自带橡胶塞；瓶口 rim 0.870、液面 0.840，
                                   橡胶塞翻放桌面倒放 0.305,0.30——同 d4l，见 flip_alkali_stopper）

用法：python scripts/gen_d4s_scene.py   （运行环境：本地 conda env 有 pxr）
"""
import math
import os
import random
...
SCENE_DIR = os.path.join(REPO, "assets", "scenes", "d_wetchem", "d4s_alkali_reagent")
OUT = os.path.join(SCENE_DIR, "d4s_alkali_reagent.usd")
...
# 皿/粉/试管/试管架坐标与 d2s 完全一致（挖粉轨迹不动）；洗瓶位换成滴管架+碱滴管+碱瓶。
...
EQUIP = [
    ...
    # —— 碱试剂（滴管进主试管架第一列第5排，与药匙同列相隔一排；碱瓶在原洗瓶附近）——
    ("DropperAlkali", "dropper.usd", (0.659, 0.4002, TUBE_BOTTOM), None, None),
    ("AlkaliBottle", "alkaline_bottle.usd", (0.370, 0.30, None), None, None),
]
...
# 碱性试剂液（d4l ALKALI：青绿色区分碱液，roughness 0.05 + ior 1.33 真液面）
...
CLOUD_MILK = dict(color=(0.82, 0.80, 0.74), opacity=1.0, roughness=0.85,
                  emissive=(1.0, 0.95, 0.80))
# 滴落液滴：碱色更亮更不透（透过玻璃可见）
DROP = dict(color=ALKALI["color"], opacity=0.90, roughness=0.05, ior=1.33)
...
# 贴管底平铺——旧 0.84 悬在碱液柱中部，白粉被不透明碱柱完全盖住。碱液柱改从粉末顶面
# （LIQUID_BASE=0.818）往上长，白粉始终露在碱液下方，倒粉后立即可见。
TUBE_SAMPLE_R = 0.006
TUBE_SAMPLE_H = 0.012
LIQUID_BASE = TUBE_BOTTOM + TUBE_SAMPLE_H   # 0.818：碱液柱底面 = 白粉顶面（粉不被碱盖）
...
def _gen_bubbles(n_center=30, n_wall=10, seed=42, cx=TUBE_CX, cy=TUBE_CY):
    ...
    cx/cy = 试管孔中心（D4-S = 0.659,0.241）。z 全写 0.806（task 每帧覆盖）。"""
...
def add_effects(stage):
    """内建效果 prim：药匙粉末 + 药粉下落 + 管内固体粉末 + 碱液柱 + 沉淀 + 浑浊云。"""
    # 药匙上粉堆（⑨ 挖粉显示，跟随勺尖）
    ...
    # 管内碱液柱（滴入后逐滴生长，task._grow_tube_level 驱动；从白粉顶面 LIQUID_BASE 往上长）
    _make_cyl(stage, "TubeDrops", 0.009, 0.030, (TUBE_CX, TUBE_CY, LIQUID_BASE + 0.015), ALKALI, False)
    # 沉淀（固体颗粒沉降，贴管底）/ 浑浊云（碱加入瞬间乳白，从液柱底往上盖）（task 几何驱动）
...
def add_color_liquid(stage):
    """候选色变色液柱（滴加碱后液体变色，d3l 同款）：/World/TubeDropsInput_<色>（输入色，
...
def add_dropper_drops(stage):
    """挤胶头滴落串：/World/DropperDrop 父 Xform + Drop_0.._N 碱色小球（r=0.003）。
...
def flip_alkali_stopper(st2):
    """碱瓶橡胶塞：从瓶口摘出，静态倒放桌面（同 d4l，无机械臂动作）。

    保留原 prim（复用 alkaline_bottle 的 rubber_stopper 几何/材质），只改 local
    xform：translate 移到碱瓶左侧 65mm、rotateXYZ 绕 y 转 180° 让大端朝下。塞子局部
    原点在几何底（小端面），世界位姿 = AlkaliBottle_T(0.370,0.30,0.7997)·S·R_y180·
    T(-0.065,0,+0.01362) → 世界原点 (0.305,0.30,0.8133)、大端触台面 0.80（实验室
...
def verify(st2):
    ...
    # 碱瓶橡胶塞断言：静态倒放桌面（同 d4l，gen 摆桌面）。大端朝下触台面 0.80、小端朝上
    # ≈0.8133（塞子几何高 13.27mm）→ 世界 bbox z 0.80..0.8133、xy 中心 ≈ (0.305, 0.30)。
    stopper = None
    for pp in Usd.PrimRange(st2.GetPrimAtPath("/World/AlkaliBottle")):
        if pp.GetName() == "rubber_stopper":
            stopper = pp
    ...
    print(f"[verify] AlkaliStopper    min({mn[0]:+.4f},{mn[1]:+.4f},{mn[2]:+.4f}) "
          f"max({mx[0]:+.4f},{mx[1]:+.4f},{mx[2]:+.4f})")
    assert abs(mn[2] - 0.800) < 2e-3, f"stopper bottom {mn[2]:.4f} not on table 0.80"
    assert abs(mx[2] - mn[2] - 0.01327) < 2e-3, f"stopper height {mx[2]-mn[2]:.4f} != 13.27mm"
    cx = (mn[0] + mx[0]) / 2.0
    cy = (mn[1] + mx[1]) / 2.0
    assert abs(cx - 0.305) < 5e-3, f"stopper center x {cx:.4f} not at 0.305"
    assert abs(cy - 0.30) < 5e-3, f"stopper center y {cy:.4f} not at 0.30"
    ...
    # 残留 DomeLight 必须清干净（纯黑环境贴图会污染整场景光照，同 d4l）
...
def main():
    ...
    add_effects(stage)
    add_color_liquid(stage)   # 候选色变色液柱（滴加碱后液体变色，初始全隐藏）
    add_bubbles(stage)
    ...
    flip_alkali_stopper(st2)     # 碱瓶橡胶塞翻放桌面倒放（同 d4l）
    brighten_spatula(st2)        # 药匙银黑不锈钢
    fix_bottle_materials(st2)    # 碱瓶不碰（塑料瓶）+ 液面盘保留
