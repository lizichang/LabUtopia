...
# v30.6 减半：用户反馈"水滴型染色火焰还是太大，改成现在的一半"。高度/半径各 ×0.5
# → 11.25mm 高 × 直径 7.5mm（≈铂丝头球 7mm，仅大一点点，回到 v30.4 初衷）。
# 配合 scripts/resize_c1_stain.py 重烘焙场景（只重建染色锥，不动管线其它环节）。
STAIN_GEOM = {
    "height": 0.01125,        # 水滴火焰高 ×0.5（11.25mm）
    "radius": 0.00375,        # 最宽处半径 ×0.5（直径 7.5mm）
}
