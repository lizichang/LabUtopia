...
- 台面前区 (0.50,0.35) 放试管架：滴管插左孔、温度计插右孔（中排，底面落孔底 z0.806）
...
# 孔心相对架原点（skill 坑33 校准）：列 x=±0.019，中排行 y=-0.0004；底层板顶(孔底) z=-0.0905 → 世界 0.806。
# 滴管(Ø8)/温度计(Ø8) 都插 Ø22.4 孔，底面落孔底 0.806。
...
EQUIP = [
    ...
    # 滴管/温度计插进架孔（左/右列，中排）；温度计 min z=-0.002 → +0.002 补偿让泡底落 0.806
    ("Dropper", "dropper.usd", (RACK_X - 0.019, RACK_Y - 0.0004, HOLE_Z), None, False),
    ("Thermometer", "thermometer.usd", (RACK_X + 0.019, RACK_Y - 0.0004, HOLE_Z + 0.002), None, False),
...
# 试管内液体（蓝半透明水柱，可见）、沸腾气泡（球组，初始隐藏）、蒸汽（球组，初始隐藏）。
...
# 蒸汽球基础位（试管口上方，task 上浮动画；以试管中心为基准）
STEAM_BASE = [
    (TUBE_X + 0.0000, TUBE_Y + 0.000, TUBE_BOTTOM_Z + 0.170, 0.010),
    (TUBE_X - 0.0040, TUBE_Y + 0.008, TUBE_BOTTOM_Z + 0.196, 0.012),
    (TUBE_X + 0.0040, TUBE_Y - 0.006, TUBE_BOTTOM_Z + 0.216, 0.014),
    (TUBE_X + 0.0000, TUBE_Y + 0.010, TUBE_BOTTOM_Z + 0.236, 0.016),
    (TUBE_X + 0.0020, TUBE_Y - 0.010, TUBE_BOTTOM_Z + 0.256, 0.018),
]


# —— 阶段B 滴加：液体材质配方（同 d3l：水透明 + ior 水折射；滴管内液柱/滴落液滴更亮更不透）——
WATER = dict(color=(0.72, 0.85, 1.0), opacity=0.50, roughness=0.05, ior=1.33)   # 样品瓶内半瓶液 / 试管内水柱
FILL = dict(color=(0.35, 0.75, 1.0), opacity=0.90, roughness=0.05, ior=1.33)    # 滴管尖内吸起液柱
...
# 滴管尖内吸起液柱（截锥 mesh：尖嘴 Ø2mm → 玻璃体直管 Ø8mm，覆盖收窄段 0..30mm + 直管 30mm）。
# 滴管插架左孔 (RACK_X-0.019, RACK_Y-0.0004)=(0.481,0.3496)，尖嘴底=孔底 HOLE_Z(0.806)。
# translate=底心贴尖嘴（与 task._set_fill_follow 同一约定：translate=尖嘴）。
DROPPER_XY = (RACK_X - 0.019, RACK_Y - 0.0004)
DROPPER_TIP_Z = HOLE_Z
FILL_R = (0.001, 0.0035)
FILL_H = 0.060
# 挤胶头滴落串：一次挤 DROPS_PER_GROUP 滴连续坠落（液柱很满，一挤该是一串滴不是一滴——
# d3l 用户 2026-08-14）；DropperDrop 父 Xform + Drop_0.._N 球，task 动画驱动
DROPS_PER_GROUP = 4
DROP_BALL_R = 0.003
...
def add_frustum(stage, name, r_bottom, r_top, h):
    ...
    （液面不贴壁、露出空隙），须用锥台贴合。16 段圆周 + 底/顶 cap，
    subdivisionScheme=none（memory: 自建 mesh 必须设，否则不显色）。
    """
    n = 16
    pts, counts, indices = [], [], []
    for i in range(n):
...
def add_b2_effects(stage):
    ...
    + 滴管尖内吸起液柱（截锥，初始隐藏，吸液后跟随尖嘴）+ 气泡组/蒸汽组（初始隐藏）。"""
    ...
    fill.AddTranslateOp().Set(Gf.Vec3d(DROPPER_XY[0], DROPPER_XY[1], DROPPER_TIP_Z))
    ...
    # 蒸汽组：5 球在试管口上方，初始隐藏，task 沸腾时 reveal + 上浮
    UsdGeom.Xform.Define(stage, "/World/Steam")
    st_prims = []
    for i, (x, y, z, r) in enumerate(STEAM_BASE):
        sp = UsdGeom.Sphere.Define(stage, f"/World/Steam/puff_{i}")
        sp.CreateRadiusAttr(r)
        sp.AddTranslateOp().Set(Gf.Vec3d(x, y, z))
        UsdGeom.Imageable(sp).MakeInvisible()
        st_prims.append(sp.GetPrim())
    add_shared_material(stage, "/World/Steam/steam_mat", (0.95, 0.97, 1.0), 0.35, st_prims)
    print(f"[effect] {len(STEAM_BASE)} steam puffs hidden")
...
def verify(st2):
    ...
    names = ["IronStand", "AlcoholLamp", "AsbestosGauze", "TestTube", "TestTubeClamp",
             ...
             "SampleLiquid", "DropperFill", "DropperDrop"]
    boxes = {}
    for name in names:
        p = st2.GetPrimAtPath(f"/World/{name}")
        if not p.IsValid():
    ...
    # 温度计：泡底落孔底（min z=-0.002 已 +0.002 补偿），x 中心对准右孔心 (RACK_X+0.019)
    thn, thx = boxes["Thermometer"]
    assert abs(thn[2] - HOLE_Z) < 0.002, f"thermo bottom {thn[2]} != hole bottom {HOLE_Z}"
    assert abs((thn[0] + thx[0]) / 2 - (RACK_X + 0.019)) < 0.003, f"thermo x center {(thn[0]+thx[0])/2:.4f} not at right hole"
    assert abs((thn[1] + thx[1]) / 2 - (RACK_Y - 0.0004)) < 0.003, f"thermo y center {(thn[1]+thx[1])/2:.4f} not at center row"
    # 滴管/温度计都在架顶板范围（x±0.043, y±0.143），确认孔位在板内
    assert dmx[0] < rmx[0] + 0.001 and thn[0] > rmn[0] - 0.001, "instruments outside rack plate x"
    # 效果 prim：试管内水柱初始隐藏 h0（阶段B 滴加后逐滴生长，上限远低于管口），气泡/蒸汽组存在
    ...
    # 滴管内吸起液柱：初始隐藏、底贴尖嘴 HOLE_Z、x 对准滴管孔心
    fl = st2.GetPrimAtPath("/World/DropperFill")
    assert fl.IsValid(), "DropperFill missing"
    assert UsdGeom.Imageable(fl).ComputeVisibility() == "invisible", "DropperFill should be hidden"
    r = bc.ComputeWorldBound(fl).ComputeAlignedRange()
    fmn, fmx = r.GetMin(), r.GetMax()
    assert abs(fmn[2] - HOLE_Z) < 0.002, f"DropperFill bottom {fmn[2]} not at tip {HOLE_Z}"
    assert abs((fmn[0] + fmx[0]) / 2 - DROPPER_XY[0]) < 0.003, "DropperFill x off tip"
    # 滴落球组：4 球、初始隐藏
    ...
    stm = st2.GetPrimAtPath("/World/Steam")
    assert bub.IsValid() and stm.IsValid(), "bubbles/steam group missing"
    nb = sum(1 for _ in bub.GetChildren())
    ns = sum(1 for _ in stm.GetChildren())
    assert nb >= 4 and ns >= 3, f"effects count bubbles={nb} steam={ns}"
    ...
    print(f"[verify] OK: 台面贴底 / 网坐环上 / 管坐网上 / 灯在网下 / 钩在柱内 / 架贴台 / 滴管+温度计插孔 / 滴加效果(管柱隐藏h0+瓶液面可见+滴柱+滴球) / 气泡+蒸汽组齐({nb}泡{ns}雾) / 皿贴台+沸石叠皿上+样品瓶(开瓶)+火柴贴台")
