# -*- coding: utf-8 -*-
"""生成 b3s_water_bath.usd —— B3 水浴加热（酒精灯加热烧杯水 → 试管内固体样品熔化）场景（烘平自包含）。
...
2026-08-29 用户修正（逐字）：
  ...
  6. 布局复刻 D2-S：「整个位置需要重新调整，先把 d2s 所有物品包含机械臂的位置复刻过来，
     ...
     → 试管/药匙/皿/粉 全用 D2-S 同款坐标（挖粉动作坐标与 d2s 完全一致，见常量区）；
...
SCENE_DIR = os.path.join(REPO, "assets", "scenes", "b_thermal", "b3s_water_bath")
OUT = os.path.join(SCENE_DIR, "b3s_water_bath.usd")
...
# 火柴 / 灯帽：2026-08-29 用户迭代位置。初版 = 架中心 (0.6803,0.3607) 正后方 -X 10cm →
# 原点 (0.5314,0.3607)；用户「火柴放在那里还是不太好，在往-x和-y各移动20cm」→ 再 -X/-Y 各
# 20cm → 原点 (0.3314,0.1607)。火柴资产原点在 -X 端（全长 0.0978），头 +X 端朝灯芯
# (0.5286,-0.25)；本位置离架/皿都远（头 0.4292 距皿 min x 0.5065 有 7.7cm）。灯帽仍 B2 相对
# 灯偏移随灯平移：帽局部 (0.1086,0.0129,-0.076) rot180 翻转 → 世界 (lamp_x-0.1086, lamp_y-0.0129)
# = (0.42, -0.2629)。
MATCH_X, MATCH_Y = 0.3314, 0.1607   # 火柴原点（架后 10cm 再 -X/-Y 各 20cm）
...
# D2-S 复刻品（药匙）rot180=True（= D2-S rotZ-180，等效）。
EQUIP = [
    ...
    # 阶段A 样品区：D2-S 布局复刻（试管先立架孔里 + 药匙挖粉 + 皿上粉丘）
...
MELT_R = 0.008                         # 试管内熔化液球半径（< 管内径 ~0.0085，贴底半圆）
# 熔化液球放浸入水浴的试管底部（管底 0.9255 = 烧杯底 0.9205 上 5mm），球心贴底上 0.006；
# 初始隐藏，task 揭示后 _set_tube_world 每帧按 _TUBE_MELT_OFFSET 写位随管刚性跟随。
MELT_CZ = BEAKER_BOTTOM_Z + 0.011      # 0.9315（浸入管底 0.9255 + 球心距底 0.006）
...
# ---- 挖粉效果 prim（复刻 D2-S，task 药匙挖粉动画驱动）----
# PowderOnSpoon 勺上粉堆（白，贴勺尖 z≈0.965）、PowderDrop 倒粉下落粉粒（父 Xform + 14 球，
# 错帧起落成细流）、TubeSample 架孔试管内白色粉末柱（倒粉落定显示）。全初始隐藏。
POWDER_COLOR = (0.93, 0.93, 0.94)
POWDER_ON_SPOON_R = 0.005
POWDER_ON_SPOON_H = 0.005
POWDER_ON_SPOON_XYZ = (SPATULA_X, SPATULA_Y, SPATULA_Z + 0.137)   # 勺尖世界 z≈0.965
POWDER_DROPS = 14
POWDER_DROP_R = 0.003
POWDER_DROP_HOME = (RACK_TUBE_X, RACK_TUBE_Y, TUBE_MOUTH_Z)       # 0.9593（管口，动画起点）
TUBE_SAMPLE_R = 0.006
TUBE_SAMPLE_XYZ = (RACK_TUBE_X, RACK_TUBE_Y, TUBE_BOTTOM_Z + 0.004)   # 管内粉末球心贴底（底部半圆）
...
def add_melt_liquid(stage):
    """试管内熔化液球：/World/TubeMelt_<色>，初始全隐藏。task 按 cfg.melt_color 揭示一根
    （sample_phase=melted 时）。球半径 MELT_R、球心贴浸入管底上 0.006（底部半圆形熔液）。"""
    for name, m in MELT_COLORS.items():
        geom = UsdGeom.Sphere.Define(stage, f"/World/TubeMelt_{name}")
        ...
        print(f"[effect] TubeMelt_{name} hidden (r{MELT_R})")


def add_scoop_effects(stage):
    """挖粉效果 prim（复刻 D2-S，task 药匙挖粉动画驱动）：勺上粉堆 + 倒粉下落粉粒
    + 管内白色粉末柱，全初始隐藏。"""
    # PowderOnSpoon 勺上粉堆（圆柱，贴勺尖，task ⑨ 挖粉时 reveal + 随勺尖 + 下落缩小）
    ps = UsdGeom.Cylinder.Define(stage, "/World/PowderOnSpoon")
    ps.CreateRadiusAttr(POWDER_ON_SPOON_R)
    ps.CreateHeightAttr(POWDER_ON_SPOON_H)
    ps.CreateAxisAttr("Z")
    ps.AddTranslateOp().Set(Gf.Vec3d(*POWDER_ON_SPOON_XYZ))
    add_material(stage, ps.GetPrim(), POWDER_COLOR, 1.0, roughness=0.5)
    UsdGeom.Imageable(ps).MakeInvisible()
    # PowderDrop 父 + 14 粉粒（倒粉下落，错帧起落成连续细流）
    UsdGeom.Xform.Define(stage, "/World/PowderDrop")
    drop_prims = []
    for i in range(POWDER_DROPS):
        sp = UsdGeom.Sphere.Define(stage, f"/World/PowderDrop/Drop_{i}")
        sp.CreateRadiusAttr(POWDER_DROP_R)
        sp.AddTranslateOp().Set(Gf.Vec3d(*POWDER_DROP_HOME))
        UsdGeom.Imageable(sp).MakeInvisible()
        drop_prims.append(sp.GetPrim())
    add_shared_material(stage, "/World/PowderDrop/drop_mat", POWDER_COLOR, 1.0,
                        drop_prims, roughness=0.5)
    # TubeSample 管内白色粉末球（倒粉落定显示，贴底半圆，task 显示）
    ts = UsdGeom.Sphere.Define(stage, "/World/TubeSample")
    ts.CreateRadiusAttr(TUBE_SAMPLE_R)
    ts.AddTranslateOp().Set(Gf.Vec3d(*TUBE_SAMPLE_XYZ))
    add_material(stage, ts.GetPrim(), POWDER_COLOR, 1.0, roughness=0.5)
    UsdGeom.Imageable(ts).MakeInvisible()
    print(f"[effect] scoop hidden: PowderOnSpoon / PowderDrop x{POWDER_DROPS} / TubeSample")
...
def verify(st2):
    ...
    烧杯直立坐网上（points 实测 90mm）、D2-S 样品区复刻（试管/药匙/皿/粉同 d2s 坐标）、
    ...
    names = ["IronStand", "AlcoholLamp", "AsbestosGauze", "Beaker", "TestTubeRack", "TestTube",
             "Spatula", "SurfaceDish", "SamplePowder", "Match", "BeakerWater"]
    boxes = {}
    for name in names:
        p = st2.GetPrimAtPath(f"/World/{name}")
        if not p.IsValid():
    ...
    # 玻璃皿贴台（D2-S 位）、粉丘贴皿顶（碗形皿凹坑内 → 底略低于皿顶缘）、火柴抬高 12mm
    ...
    for name in MELT_COLORS:
        ...
        assert abs(UsdGeom.Sphere(mp).GetRadiusAttr().Get() - MELT_R) < 1e-9, f"TubeMelt_{name} r != {MELT_R}"
    # 挖粉效果 prim：勺上粉堆/倒粉粉粒/管内粉末柱齐且全隐藏（复刻 D2-S）
    for path in ("/World/PowderOnSpoon", "/World/TubeSample"):
        p = st2.GetPrimAtPath(path)
        assert p.IsValid(), f"{path} missing"
        assert UsdGeom.Imageable(p).ComputeVisibility() == "invisible", f"{path} should be hidden"
    pd = st2.GetPrimAtPath("/World/PowderDrop")
    assert pd.IsValid(), "PowderDrop missing"
    nd = sum(1 for c in pd.GetChildren() if c.GetTypeName() == "Sphere")
    assert nd == POWDER_DROPS, f"powder drops count {nd} != {POWDER_DROPS}"
    assert UsdGeom.Imageable(st2.GetPrimAtPath("/World/PowderDrop/Drop_0")).ComputeVisibility() == "invisible", \
        "powder Drop_0 should be hidden"
    ...
    print(f"[verify] OK: 台面贴底 / 网坐环上 / 烧杯直立坐网上(90mm,堆叠移y={STAND_Y}) / 水柱在杯内 / "
          f"D2-S样品区复刻(架/管/勺/皿/粉) / 挂钩隐藏 / 无试管夹 / 灯在网下 / 帽随灯移 / "
          f"火柴抬高12mm / 水浴气泡组齐({nb}泡)隐藏 / 熔化液柱5根隐藏 / 挖粉效果(勺粉/粉粒/管内粉)齐隐藏"
          f" / 火焰迁/World顶层")
