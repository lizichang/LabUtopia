...
资产清单：
  test_tube      试管 10mL      外径19.2mm 壁厚1.5mm 高153mm 圆底 玻璃（Ø15×120 放大 1.2779 固化）
...
（test_tube_rack / wash_bottle 形状错误，已按用户要求从生成清单移除。
 试管尺寸：用户 temp_d2s 设定放大 1.2779（Ø15->Ø19.2mm 好放进试管架 Ø22.4mm 孔），
 已固化进 assets/equipment/test_tube.usd 与 *_simple.usd，场景不得再放大。）
...
def build_test_tube(mb):
    S = 40
    # 外壁：圆底(0->0.0051 圆角) + 直管 0.0051->0.1533
    # （Ø19.2mm×153mm = 原 Ø15×120 放大 1.2779，用户 temp_d2s 设定，勿改回）
    mb.lathe([(0.0000, 0.0000), (0.0058, 0.0010), (0.0083, 0.0028),
              (0.0096, 0.0051), (0.0096, 0.1533)], S, "tube")
    # 内壁（壁厚 1.5mm -> 内径 16.2mm），reverse 法线朝内
    mb.lathe([(0.0081, 0.0051), (0.0081, 0.1533)], S, "tube", reverse=True)
    # 底部内环带（连接外壁内底与内壁底，法线朝下）
    mb.lathe([(0.0096, 0.0051), (0.0081, 0.0051)], S, "tube")
    # 口部环带（连接内壁口与外部口，法线朝上）
    mb.lathe([(0.0081, 0.1533), (0.0096, 0.1533)], S, "tube")
