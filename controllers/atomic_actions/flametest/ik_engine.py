...
class IkMotionEngine:
    ...
    # 臂移动更慢更稳，惯性/穿模消失。垂直段 VZ_STEP 的每帧关节量远小于此，不受影响。
    # 注：曾试加速度斜坡（MAX_JOINT_ACCEL 速度梯形），臂反而不稳（起停缓+到点逼近抖动），
    # 用户 2026-08-14 要回退——保持直接钳制（无穷加速度封顶即速度封顶）。
    # 2026-08-14 晚：用户「动作大体没问题但边做边晃、没有刚刚流畅」。运动代码与流畅那轮
    # 逐字节一致，回归来自药匙勺头扁平面转到沿 X（宽面 22mm 正对 camera1）——臂末端的
    # 微振荡（直接钳制 PD 起停冲击 + 逐帧 IK 重解）之前被勺头侧对镜头（4.8mm 薄边）藏住，
    # 现在被宽面放大成可见"晃"。继续降速：0.008→0.006 ≈ 0.36 rad/s，缩小每个速度阶跃的
    # 激励幅度（VZ_STEP 同步 0.002→0.0015 保持 4:1，垂直段关节量仍在钳制内不拖滞）。
    MAX_JOINT_DELTA = 0.006
