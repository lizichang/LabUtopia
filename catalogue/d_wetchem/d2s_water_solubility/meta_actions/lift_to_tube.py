"""LiftToTube：⑨ 挖粉后，竖直抬升到试管管口上方 10cm（锁 x/y + 保持世界朝向，只变 z）。

用户 2026-08-24（逐字）：「所以现在增加动作，将爪子抬升到试管管口高2cm（只能z变，其他不要变）」；
同日「把第10步的2cm改成10cm」→ 改 10cm。
...
- 位置：x/y 锁首帧当前值（x=0.537 已对齐粉堆、y=0.2008 ⑧ 平移后），仅 z 升到
  LIFT_TUBE_Z = 管口顶 0.9593 + 10cm = 1.0593（绝对目标，与运行时当前 z 无关）。
  MoveAction 自动判垂直段（x/y 锁目标值、z 逐帧推进 VZ_STEP、TCP 走严格竖线，v46 机制，
  与 flametest 提出/下降同）。
- 复用 MoveAction 已验证逻辑：单轴直线 + 冻结（位置 <1cm 连续 3 帧 + 朝向收敛）+ dwell +
  夹爪每帧发 grip_target。

完成后：TCP (0.537, 0.2008, 1.0593)——比⑨结束抬升 15.43cm，高于管口顶 0.9593 共 10cm；
...
class LiftToTube:
    """保持当前世界朝向，竖直抬升到管口上方 10cm，x/y 锁当前值。"""
    ...
    def forward(self, joints, gripper_pos, grip_target):
        if self._move is None:
            ...
            pos = np.array([gp[0], gp[1], self.z])   # x/y 锁当前值，z 升到管口上方 10cm
