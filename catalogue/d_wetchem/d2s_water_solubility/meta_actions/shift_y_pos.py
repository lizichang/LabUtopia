"""ShiftYPos：⑩ 抬升后，往 +Y 平移 18cm（锁 x/z + 保持世界朝向，只变 y）。
...
（紧接着 ⑫ 再往 +X 移 10cm；⑪⑫ 合起来把药匙从粉丘上方水平挪到试管口近前。）

- 朝向：首帧 fk_pose 采样当前**实际**工具朝向（= ⑩ 抬升后的法兰 -90° 朝向，药匙水平）
  → 转引擎 [w,x,y,z]（scalar-first）→ 整段保持，即"世界里绝对朝向不变"。与 ⑥⑦⑧ 同：
  不硬编码朝向常数（ORIENT_SCOOP 坑）。
- 位置：x/z 锁首帧当前值（x=0.537 已对齐粉堆、z=1.0593 ⑩ 抬升后），仅 y 增
  Y_SHIFT_POS (0.18m)。MoveAction 自动判单轴水平段（仅 y 变 → y 逐帧推进、x/z 锁目标值、
  TCP 走严格直线，v47 机制）。
- 复用 MoveAction 已验证逻辑：单轴直线 + 冻结（位置 <1cm 连续 3 帧 + 朝向收敛）+ dwell +
  夹爪每帧发 grip_target。

完成后：TCP (0.537, 0.3808, 1.0593)；法兰 -90° 水平时勺尖 = TCP + 0.134·(0,-1,0) =
(0.537, 0.2468, 1.0593)（管口中心 y=0.241 后 5.8mm，用户 2026-08-24「倒数第二步y再增加1cm」）。
x 仍 0.537（x 由⑫ +10cm 跟进到 0.637，管口 x=0.659，尚差 2.2cm，供后续倾倒段对齐）。
"""
import numpy as np
...
class ShiftYPos:
    ...
    def forward(self, joints, gripper_pos, grip_target):
        if self._move is None:
            ...
            pos = np.array([gp[0], gp[1] + self.shift, gp[2]])   # x/z 锁当前值，y 增 18cm
