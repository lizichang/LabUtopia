"""元动作 ①：水平夹取药匙 → 竖直提起过架顶 → 直接转腕关节放平。

用户动作要求（逐字）：「不是说夹的时候立刻把药匙旋转90度 我的意思是首先水平夹住
然后直接竖直向上提起来，直到药匙底部高于试管架的时候，通过机械臂的旋转把药匙变成
水平的（这也说明爪子从水平现在变成竖直了）」
简化（逐字）：「直接让爪子关节旋转90度由水平变成竖直不就可以了」

新握持（task._T_HELD = R_z90·R_y90+t(0.112,0,0)）让药匙长轴 = 夹爪局部 X、与手指
垂直：手指水平(+X)时药匙竖直挂在下、与架内竖插姿态零跳变——夹住**不旋转**，竖直
提起也**不旋转**；提到勺尖过架顶 0.917（SPAT_LIFT_Z=1.15，勺尖 = 1.15-0.134 =
1.016 > 0.917）后**直接转腕关节**把手指从水平转到竖直、药匙随之放平（勺头朝 +X
远离机械臂）。

⑥ 为何不用 MoveAction 的 orient=ORIENT_FLAT：那是在**冻结 TCP** 下让 Lula 重解整个
手臂 = 全臂 IK 最难的退化情形，解不出 → 每帧保持不动 → 600 帧 force-done → 视频里
看不到旋转（2026-08-14 实测根因）。WristFlipAction 只驱动腕关节簇（索引 3/4/5）
逐帧贪心推进，不依赖 IK 收敛，腕限位内必达（合成验证 99 帧放平、手不下沉、勺尖只升）。
...
轨迹（TCP 世界坐标，x 均取 +X 侧）：
  ...
  ⑤ 竖直提起       mv((sx,sy,SPAT_LIFT_Z), orient=ORIENT_HORIZ)   # 药匙仍竖直，勺尖过架顶 1.016>0.917
  ⑥ 转腕放平       WristFlipAction(e)                             # 爪子水平→竖直（腕关节直转），药匙变水平
"""
from ._base import BaseMetaAction, mv, grip
from .constants import (H, GRIP_SPATULA, ORIENT_HORIZ,
                        SPAT_LIFT_Z, SPAT_XY, SPAT_APPROACH, SPAT_GRASP)
from .wrist_flip import WristFlipAction
...
class PickSpatula(BaseMetaAction):
    def _build_actions(self):
        ...
        return [
            ...
            WristFlipAction(e),                                       # ⑥ 直接转腕关节：爪子水平→竖直，药匙放平
        ]
