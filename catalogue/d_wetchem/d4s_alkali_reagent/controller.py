...
注册顺序：①PickSpatula（挖粉倒粉）→ ②ReturnSpatula（药匙放回）→ ③AlkaliPass（吸碱滴碱）
→ ④TubeShakePass（拿起试管震荡）。加碱滴入后固体样品+碱混合触发现象（气泡/沉淀/变色）。
...
from .meta_actions import PickSpatula, ReturnSpatula, AlkaliPass, TubeShakePass
from .meta_actions.constants import GRIP_OPEN, SHAKE_CENTER_TCP, SPAT_XY, SCOOP_ANCHOR_Y


class D4SAlkaliReagentTaskController(TaskBaseController):
    """Composite controller: 整个 D4-S 实验 = 元动作的顺序执行。"""
    ...
    def _init_collect_mode(self, cfg, robot):
        ...
        print("[d4s] controller VERSION v1 (meta-actions: PickSpatula->ReturnSpatula->AlkaliPass->TubeShakePass, IK-driven)")
        ...
        # 元动作：①PickSpatula（挖粉倒粉）→ ②ReturnSpatula（药匙放回）→ ③AlkaliPass（吸碱滴碱）
        # → ④TubeShakePass（拿起试管震荡混合），顺序执行。碱滴管一次持握内循环
        # 「吸碱-滴碱」cfg.alkali_cycles 遍（抓一次→多遍滴→放回一次，中途不松开；管内多积
        # 几滴碱液，液面逐滴升高；加碱滴入后固体样品+碱混合触发现象）。挖粉/震荡复用 d2s。
        alkali_cycles = max(1, int(getattr(cfg, "alkali_cycles", 1)))
        shake_cycles = max(1, int(getattr(cfg, "shake_cycles", 3)))
        self.meta_classes = [PickSpatula, ReturnSpatula, AlkaliPass, TubeShakePass]
        self.meta_names = [
            "S1 pick spatula + scoop powder + pour into tube (d2s, coords unchanged)",
            "S2 return spatula to rack (d2s)",
            f"A alkali aspirate+drip into tube x{alkali_cycles} (horizontal grip, ORIENT_FWD)",
            f"T shake tube x{shake_cycles} (d2s)",
        ]
        # AlkaliPass/TubeShakePass 带 cycles 参数，不能再用 [C(self.engine) for C in ...] 统一构造
        self.meta_actions = [
            ...
            # lift_first=True：D4-S 家用离⑬倒粉位仅 2.3cm，回程水平段太短调不直（残留倾斜带进
            ...
            # 震荡抬高到 1.18（d4s 自用，用户"震荡抬高一点再震荡"）：管底清架顶 12cm、
    ...
    def _step_collect(self, state):
        if self._meta_idx >= len(self.meta_actions):
            print("[d4s] all meta-actions done. success.")
        ...
        if self._start:
            ...
            if meta.is_done():
                print(f"[d4s] meta {self._meta_idx} done: {self.meta_names[self._meta_idx]}")
                self._meta_idx += 1
                if self._meta_idx < len(self.meta_actions):
    ...
    def get_language_instruction(self):
        return ("Scoop the solid powder sample with the spatula and pour it into the "
                "test tube in the rack, return the spatula, aspirate the alkali reagent "
