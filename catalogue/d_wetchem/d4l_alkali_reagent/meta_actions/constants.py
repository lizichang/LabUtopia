"""D4-L 碱性试剂滴加反应（液体样品）元动作共享常量。

所有坐标 = TCP（right_gripper / tool_center）世界坐标，米，Z-up（桌面 z=0.80）。
几何实测来源：2026-08-24 pxr 读 d4l_alkali_reagent.usd 世界包围盒（gen_d4l_scene.py
verify 输出）：
  ...
  DropperSample  (0.2815,-0.1187,0.806)  后排左孔（离试管最远，用户 2026-08-14 调整），
                                         尖嘴底=原点，胶头顶 z=0.9562
  DropperAlkali  (0.3202,-0.1187,0.806)  后排右孔
  SampleBottle   (0.4045,0.3585)  瓶口 rim z=0.870，瓶内液面 z=0.840（半瓶）
  AlkaliBottle   (0.1696,0.361)   瓶口敞（rim z=0.870）；橡胶塞 rubber_stopper 由
                                  gen 翻放桌面 (0.1046,0.361) 倒放（大端朝下触台面
                                  0.80、小端朝上 0.81327）——D4-L 无机械臂拔塞动作
  （D3-L 同款布局，仅 HClBottle → AlkaliBottle；碱瓶塞 gen 静态倒放，同 D3-L 瓶盖模式）
...
（D4-L 碱瓶橡胶塞由 gen 静态倒放桌面，无机械臂持握，故无塞子抓点常量。）
"""
# —— 高度 / 停留 ——
...
# —— 加碱滴管（/World/DropperAlkali，后排右孔）——
DROP_ALKALI_XY = (0.3202, -0.1187)
DROP_ALKALI_GRASP = (0.3202, -0.1187, DROP_SAMPLE_GRASP[2])
DROP_ALKALI_REST = (0.3202, -0.1187, 0.806)
...
# —— 碱性试剂瓶（/World/AlkaliBottle (0.1696,0.361)，口 rim 0.870）——
# 碱瓶自带 rubber_stopper 橡胶塞盖瓶口（小端插入 z 0.8616..0.870、大端露出 z 0.870..0.8748）。
# 吸碱液前拔塞倒放（StopperOpenPass），加碱后再盖回（StopperClosePass）。
ALKALI_BOTTLE_XY = (0.1696, 0.361)
ALKALI_SQUEEZE_TCP = (ALKALI_BOTTLE_XY[0], ALKALI_BOTTLE_XY[1], BOTTLE_SQUEEZE_TCP[2])  # = 1.005
ALKALI_DIP_TCP = (ALKALI_BOTTLE_XY[0], ALKALI_BOTTLE_XY[1], SAMPLE_DIP_TCP[2])          # = 0.960
...
SHAKE_PERIOD = 60             # 一个来回帧数（60Hz ≈1s/来回）。2026-08-16 用户：震荡 3→5 次、要更快
                              ...
                              #   空气里，不碰架体/试管
# 用户 2026-08-14：震荡来回后在高位**停留 5 秒**再放回；现象从加碱滴入即出现、
...
EFFECT_BUBBLES = "/World/Bubbles"            # 气泡（cfg.has_bubbles；task 用 _bubbles_path 选色组）
EFFECT_DROPPER_FILL = "/World/DropperFill"   # 滴管尖内截锥液柱（吸液后显示，跟随尖嘴；
                                             #  几何在 gen_d4l_scene.py，task 只需 translate=尖嘴）
