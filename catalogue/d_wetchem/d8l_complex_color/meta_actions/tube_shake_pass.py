...
V7 步9（边加边振摇）：三支滴管分别滴完样品/试剂 1/试剂 2 后，抓起试管震荡让液体混合
（本实现放在最后一支滴管之后作为收尾一步）。

抓试管：试管立插前排左孔，管口 z=0.9593、管高 0.153m、架顶 0.917——管身露在架上的
可握段只有 42mm（0.917..0.9593），抓**管口下 14mm**（TCP z≈0.9453）。持握 = TCP +
TUBE_HELD_OFFSET(0,0,-0.139)（管底 0.139m 吊在夹爪下方，task 纯平移保竖立），管内
液柱随管平移。夹爪开度 GRIP_TUBE≈Ø19.2mm/2=0.0096。

轨迹（TCP 世界坐标，手指默认朝下）：
  ① 高位接近试管上方   mv((tx,ty,H))
  ② 垂直下探抓管身中段 mv(TUBE_GRASP_TCP)          # 管口下 14mm（task: tube attached）
  ③ 合爪夹紧           grip(GRIP_TUBE, 60)
  ④ 垂直提出到震荡高度 mv(SHAKE_CENTER_TCP, 5)
  ⑤ 震荡来回 N 下       shake(center, cycles=N)
  ⑥ 震荡后停留 5 秒     mv(SHAKE_CENTER_TCP, SHAKE_HOLD_FRAMES)  # 高位悬停（分层/沉淀沉降）
  ⑦ 垂直下探回架       mv(TUBE_GRASP_TCP)
