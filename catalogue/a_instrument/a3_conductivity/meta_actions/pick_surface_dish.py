...
碗壁，口沿 clip 3mm）。抓取 TCP 高度 0.8620（tool_center 比指端高 0.027 → 指端 0.835 进天平机身顶
5mm——无碰撞仅接近时短暂穿入；皿底 0.8474 在指端上方 12.4mm、皿中心高出指腹正中——四改：皿偏上）。

task 侧：皿 rest → 近抓点+合爪 → attached（纯平移跟随，皿原点=TCP−0.0146）→ 开爪 → released
