"""元动作 ⑤：取灯帽盖灭火焰（2026-09-02 用户「拿起酒精灯帽盖上酒精灯熄灭火焰，可以仿照
其他实验」）。

阶段 ④ SpoonToFlamePass 燃烧 10s 放回燃烧匙后，机械臂从桌面取灯帽（灯正北 +y 12cm）、移到
灯口上方扣下盖灭火焰（照 C4 CapLampPass / B2 CapLampPass 逐字移植）。

C3 布局：帽 /World/AlcoholLamp/cap（灯子 prim）Ø37mm×3.1cm 开口朝下倒扣桌面，帽中心
静止位 (0.30,0.50,0.8155)（灯正北 +y 12cm，gen CAP_DETACH translate(0,-0.12,-0.0762) 随
灯 R180Z → 帽世界 y=灯y+0.12）。盖严实 = 帽中心 0.8915（帽底 0.8760 < 灯体顶 0.8897，
帽 local translate → (0,0,0) = 资产原始帽位），夹爪 CAP_BURNER 0.900。

照 C4：夹帽/全程默认朝向手指朝下（低 z 桌面夹帽 ORIENT_FWD 手指朝前 Lula 无解）；纯平移
持握（帽中心 = 夹爪 + CAP_HELD_OFFSET），帽竖直开口朝下不旋转。火焰熄灭时机照 B2 十一改：
帽下降罩过火焰顶（CAP_EXTINGUISH_Z 门控）才熄，不移动时早灭。
...
流程（一次持握，无循环，全程默认朝向手指朝下）：
  ...
  ⑥ 运到灯口上方：mv((bx,by,CAP_HIGH))（灯口在 (0.30,0.38)，-y 12cm 直进）。
