"""FlangeRollAction：只动最后一个关节（panda_joint7，索引 6 = 法兰自转），转到当前-45°。
...
2026-08-20 用户：「改成60度」；2026-08-22 用户：「把法兰转的-60改成-50」；2026-08-23 用户：
「-50 改成 -45」——转 45°（比 -90°转得少，药匙保持 45° 倾斜、勺头低柄高，方便勺凹碰到粉丘）。
方向不变（2026-08-14 晚已确认 +90° 转反，改 -90°，方向沿用）。

几何（pxr 行向量验证）：pick 以 ORIENT_FWD 夹起后药匙竖挂（长轴=世界 Z、勺头朝下）；
joint7 绕 tool+Z（=世界 +X，朝向 camera1）自转 → 药匙长轴从世界 Z 向水平转。
-90° = 完全放平（水平）；-45° = 45° 倾斜（长轴到 (0,-0.707,-0.707) 方向，勺头低）。纯关节命令、
不重解 IK，避免全臂 IK 在冻结 TCP 下重解的退化情形（wrist_flip 注释）。
...
JOINT = 6
LIMIT = 2.8973
ANGLE = -np.pi / 4       # -45°（用户 2026-08-23 把 -50° 改成 -45°；2026-08-14 已定方向为负）
...
class FlangeRollAction:
    """只动最后一个关节转 -45°，到位后 hold dwell 帧完成。"""
