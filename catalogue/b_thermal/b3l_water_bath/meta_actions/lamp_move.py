...
流程（9 步，一次持握，无循环）：
  ...
  ④ +X 移入：mv(LAMP_GRASP) 到灯体中心（freeze_dist 默认 0.010 精确对齐）。
  ⑤ 停顿稳定：hold(SETTLE)。
  ⑥ 合爪夹住灯体：grip(GRIP_LAMP, 60) → task 检测 attached（灯写 _LAMP_HELD·tool_world）。
  ⑦ 水平移灯：mv(LAMP_TARGET, 40)（纯 +y 移 20cm，单轴 y 稳）。
...
class LampMovePass(BaseMetaAction):
    ...
    def _build_actions(self):
        ...
        return [
            ...
            mv(e, LAMP_GRASP, orient=ORIENT_FWD),      # ④ 只水平 +X 移入 灯体中心（freeze_dist 默认 0.010 精确对齐）
            hold(e, SETTLE),                     # ⑤ 停顿稳定（合爪前多停稳）
            grip(e, GRIP_LAMP, 60),              # ⑥ 合爪夹住灯体最宽处（task 检测 attached）
            # ⑦ 移灯（2026-09-01 回退 freeze_dist=0.03 与拆两步）：④ 冻在 1cm 内 → attach 时 x
            # 误差 <AXIS_EPS(0.015) → ⑦ 自动判定纯单轴 y linewalk 稳，不需拆 x/y 两步。
            mv(e, LAMP_TARGET, 40, orient=ORIENT_FWD),  # ⑦ 纯 +y 移 20cm（xz/朝向不变）
