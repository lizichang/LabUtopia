"""元动作 ⑧：盖灯帽灭火（B3 水浴加热 阶段F，复刻 B2 CapLampPass）。

用户逐字（2026-08-29）：「熄灭酒精灯应该先把酒精灯往+y方向移动20cm(参考b2)，然后再盖上
灯冒」——机械臂从桌面取灯帽、移到灯口上方扣下盖灭火焰。灯已由 LampMovePass 先 +Y 移 20cm
到 (0.5286,-0.05)，故 CAP_BURNER 直指移灯后的灯口 (0.5286,-0.05,0.900)（非灯原位）。
其余（夹帽默认朝向手指朝下、纯平移持握、盖到位熄火、帽锁灯口）全部照搬 B2。

几何（2026-08-30 pxr 实测，B2 复用）：帽 = /World/AlcoholLamp/cap（灯子 prim），
Ø37mm×3.1cm 竖直开口朝下倒扣桌面。帽静止位 CAP_REST=(0.42,-0.2629,0.8155)（灯原位 -X
侧桌面；移灯期间 task 每帧钉帽在此，不随灯滑走）；盖灭时帽中心 0.8917（盖严实，同资产
原始帽位：帽底 0.8762 = 灯z0.8002+0.0760，套住 holder_top 0.8912 的灯口），夹爪
CAP_BURNER 0.900 = 帽中心 + 持握偏移 0.0083。

运帽路径（CAP_HIGH=1.00）离烧杯中心 (0.5286,-0.25) 最近 9cm > 烧杯半径 3.75cm → 不穿烧杯
（灯移走后烧杯仍在原位，运帽从烧杯西北侧外绕到移灯位正上方再下扣）。
...
class CapLampPass(BaseMetaAction):
    ...
    def _build_actions(self):
        ...
        return [
            ...
            mv(e, CAP_GRASP, freeze_dist=0.03, timeout=200),  # ② 竖直下探夹帽（低z手腕近桌面IK死区→near即freeze+4s兜底）
