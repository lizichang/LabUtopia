"""E2 磁性检测控制器：顺序执行元动作（夹磁铁检测 → 撤回磁铁）。

与 d2s/flametest 同构（Lula IK + 元动作组合）：
  - atomic_actions/flametest/（IK 原子动作，Lula IK 驱动）
  - meta_actions/（一个 v11 步骤 = 一个元动作，一类一文件）
  - 本控制器：实例化元动作按序 forward()，全部完成 → success。

2026-08-26 用户简化设计（待测固体预铺在表面皿，删药匙/试管架/样品瓶/瓶盖）→
2 个元动作：
  ① PickMagnet   夹条形磁铁 → 缓慢靠近皿上方样品做磁性检测（task 期间驱动磁性颗粒吸起动画）
  ② ReturnMagnet 撤回磁铁归位
"""
import os
import numpy as np
...
from .meta_actions import PickMagnet, ReturnMagnet
from .meta_actions.constants import GRIP_OPEN


class E2MagneticTaskController(TaskBaseController):
    """Composite controller: 整个 E2 实验 = 元动作的顺序执行。"""
    ...
    def _init_collect_mode(self, cfg, robot):
        ...
        print("[e2] controller VERSION v3 (meta-actions: PickMagnet/ReturnMagnet, Lula IK-driven)")
        self.orient = euler_angles_to_quat(np.array([0, np.pi, 0]))
        # Lula IK 求解器（同 d2s/flametest）
        ...
        self.meta_classes = [PickMagnet, ReturnMagnet]
        self.meta_names = [
            "S1 pick bar magnet and lower over pre-placed sample for detection (grain-rise anim)",
            "S2 return magnet to table",
        ]
    ...
    def _step_collect(self, state):
        if self._meta_idx >= len(self.meta_actions):
            print("[e2] all meta-actions done. success.")
        ...
        if self._start:
            ...
            if meta.is_done():
                print(f"[e2] meta {self._meta_idx} done: {self.meta_names[self._meta_idx]}")
                self._meta_idx += 1
                if self._meta_idx < len(self.meta_actions):
    ...
    def get_language_instruction(self):
        return ("Pick up the bar magnet and lower it slowly over the solid sample already "
                "placed on the surface dish to detect magnetic attraction; then return the "
                "magnet to the table.")
