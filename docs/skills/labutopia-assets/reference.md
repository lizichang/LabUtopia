...
### 机械臂可达性与操作朝向（布局设计，先布局后救火）

**坑 34 详解：场景布局在设计阶段（写 gen_XXX_scene.py 的 REFS 之前）就必须满足机械臂的操作需求，不是卡住/伸不开才挪底座救火。** 用户 2026-08-25 明确纠正：B2 初版把铁架台放得太靠臂+朝向反，机械臂根本伸不开、没法把温度计挂上钩、没法滴加——用户手动把铁架台**整体移远**并**绕 Z 转 180° 面向臂**。这类"最初构架场景就没考虑臂"的问题反复出现，必须在布局时前瞻检查。

**布局三步：**
1. **定底座**：config `robot.position`（`np.array(cfg.robot.position)` 驱动底座摆放）。向后 = -Y（D2-S 实测）。D2-S 最终 `[0.05, 0.17, 0.71]`。
2. **距离约束**：器材**不能落底座盲区**（底座周围一小圈伸不开/挡臂），也**不能超臂展**（Franka 0.855m，最远物品必须在内）。B2：初版铁架台离臂太近→臂伸不开→用户整体移远（铁架台到 x≈0.629，堆叠中心 x≈0.529）。
3. **朝向约束**：**被操作部位朝机械臂**。B2：铁架台初版朝向反→机械臂够不到挂温度计的钩、滴加方向不合理→用户整组绕 Z180° 转向臂。堆叠件（灯/网/管/夹）整组转 180°，操作侧（管口/夹/钩）朝臂。

**从操作语义反推**：列这个实验机械臂做的每个操作（挂温度计=温度计在架上、钩朝臂；滴加=试管口在臂侧；夹取=夹柄/孔朝臂），布局决定各 approach 方向，写进 REFS 坐标。与 labutopia-task「先理解步骤物理语义」是同一思想，只是发生在布局层。

**用户重排后的联动**（scene-realign 收尾项）：器材整体移动后工作区相对底座变化大时，`robot.position` 可能要跟；距离判断 = 最近器材在底座盲区外、最远器材 ≤0.855m。
